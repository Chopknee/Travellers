//Script grabbed from Sebastian Lague's video

using UnityEngine;

public class Erosion {

    //ErosionData erosionData;//Also added this
    ...
    public Erosion(ErosionData data, int seed) {
        //erosionData = data;
        this.seed = seed;
        erosionRadius = data.erosionRadius;
        inertia = data.inertia;
        sedimentCapacityFactor = data.sedimentCapacityFactor;
        minSedimentCapacity = data.minSedimentCapacity;
        erodeSpeed = data.erodeSpeed;
        depositSpeed = data.depositSpeed;
        evaporateSpeed = data.evaporateSpeed;
        gravity = data.gravity;
        maxDropletLifetime = data.maxDropletLifetime;
        initialWaterVolume = data.initialWaterVolume;
    ...
    public void River( float[] map, int mapSize, float startSediment = 0, float riverCount = 1, int numIterations = 1, bool resetSeed = false ) {
        Initialize(mapSize, resetSeed);
        for (int river = 0; river < riverCount; river++) {
            float rX = prng.Next(0, mapSize - 1);
            float rY = prng.Next(0, mapSize - 1);
            for (int iteration = 0; iteration < numIterations; iteration++) {
                float posX = rX + ( 5 - prng.Next(0, 10) );
                float posY = rY + ( 5 - prng.Next(0, 10) ); 
                // Create water droplet at random point on map
                float dirX = 0;
                float dirY = 0;
                float speed = initialSpeed;
                ...
                for (int lifetime = 0; lifetime < maxDropletLifetime; lifetime++) {
                    ...
                    if ( posX < 1 || posX >= mapSize - 2 || posY < 1 || posY >= mapSize -2) {
                        break;
                    }
